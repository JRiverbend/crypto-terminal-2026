...
def get_crypto_prices():
    fallback = {"bitcoin": {"usd": 68400.0, "usd_24h_change": 0.0}, "the-graph": {"usd": 0.0236, "usd_24h_change": 0.0}}
    try:
...
def get_mempool_data():
    try:
        # Анализ 50 последних транзакций для вычисления индекса сентимента
        res = requests.get("https://api.mempool.space/api/mempool/recent", timeout=5).json()
        
        # Фильтруем только КРУПНЫЕ транзакции (> 5 BTC)
        whale_alerts = [tx for tx in res if (tx['value'] / 10**8) >= 5.0]
        
        # Динамический расчет Sentiment Score
...
st.caption(f"User: JRiverbend | Netanya HQ | Updated: {datetime.now().strftime('%H:%M:%S')}")
...
with left_col:
    ...
    if not whale_txs:
        st.info("Крупных движений в мемпуле сейчас нет. Сеть спокойна.")
    else:
        for tx in whale_txs:
            val = tx['value'] / 10**8
    ...
    st.caption("Данные из Mempool.space. Обновляются при рефреше страницы.")

with right_col:
    st.subheader("📰 Market Intelligence")
    news = get_blockchain_news()
    if not news:
        st.write("Лента новостей пуста или недоступна.")
    for entry in news:
        st.markdown(f"""
        <div class="news-card">
            <a class="news-title" href="{entry.link}" target="_blank">{entry.title}</a><br>
            <small style="color: #8b949e;">{entry.get('published', 'Today')}</small>
        </div>
        """, unsafe_allow_html=True)
